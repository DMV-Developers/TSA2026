...
public class PrometeoCarController : MonoBehaviour
{
    ...
    */
    ...
    private PlayerInput.PlayerActions controls;
    ...
    private void Awake()
    {
        ...
        controls = inputasset.Player;
        controls.Enable();

        controls.Throttle.performed += OnThrottle;
        controls.Throttle.canceled += OnThrottle;
        controls.Steering.performed += OnSteer;
        controls.Steering.canceled += OnSteer;
        controls.Handbrake.performed += OnHandbrake;
        controls.Handbrake.canceled += OnHandbrake;
    }
      ...
      */
      if (useTouchControls && touchControlsSetup){
        ...
        if (!isAI)
        {
          ...
          if((!isSPressed && !isWPressed) && !isSpacePressed && !deceleratingCar){
            InvokeRepeating("DecelerateCar", 0f, 0.1f);
            deceleratingCar = true;
          }
          if(!isAPressed && !isDPressed && steeringAxis != 0f){
            ResetSteeringAngle();
          }
        }
        else
